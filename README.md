...
$$output\_ height = \frac{{input\_ height - kernel\_ size + 2 \times padding}}{{stride}} + 1$$

$$output\_ width = \frac{{input\_ width - kernel\_ size + 2 \times padding}}{{stride}} + 1$$

```
Example of an intermediate output: 
The input layer is the Red Channel fo the `input`, `kernel_stride` = 1, `padding` = 0
```
